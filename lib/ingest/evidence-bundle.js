 ...
 * @returns {boolean}
 */
export function shouldUseFallbackRulebook(bundles, threshold = DEFAULT_EVIDENCE_FALLBACK_SCORE_THRESHOLD) {
  return bestEvidenceScore(bundles) < threshold;
}
...
export function computeHybridScore(vectorScore, strictFts, orFts, question, ruleNumber) {
  ...
  const rule = String(ruleNumber ?? '').trim();
  const ruleBoost = rule && question.toLowerCase().includes(rule.toLowerCase()) ? 0.15 : 0;
...
export async function fetchEvidenceBundlesWithFallback(dbClient, primaryVersionId, question, opts = {}) {
  ...
  const needsFallback = Boolean(
    ...
    shouldUseFallbackRulebook(primary.bundles, scoreThreshold),
  );
