      ...
      const res = await dbClient.query(`
        WITH vector_candidates AS (
          SELECT
            ...
            ts_rank(c.search_vector, plainto_tsquery('english', $1)) AS strict_fts_score,
            CASE
              WHEN $5::text = '' THEN 0
              ELSE ts_rank(c.search_vector, to_tsquery('english', $5))
            END AS or_fts_score
          FROM rule_node_chunks c
          JOIN rule_nodes n ON n.id = c.rule_node_id
          WHERE c.extraction_run_id = $2
            AND n.rulebook_version_id = $3
        ...
        SELECT *,
          (
            ...
            CASE WHEN lower($1) LIKE '%' || lower(coalesce(rule_number, '')) || '%' THEN 0.15 ELSE 0 END
          ) AS hybrid_score
    ...
    if (chunkHits.length === 0) {
      const lexical = await dbClient.query(`
        SELECT
          ...
          n.page_end,
          greatest(
            ts_rank(c.search_vector, plainto_tsquery('english', $1)),
            CASE
              WHEN $5::text = '' THEN 0
              ELSE ts_rank(c.search_vector, to_tsquery('english', $5))
            END
        ...
        WHERE c.extraction_run_id = $2
          ...
          AND (
            ...
            OR ($5::text <> '' AND c.search_vector @@ to_tsquery('english', $5))
          )
