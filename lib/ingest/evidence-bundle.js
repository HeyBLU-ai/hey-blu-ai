...
/**
 * @param {string} question
 * @param {string} ruleNum
 * @returns {boolean}
 */
export function questionReferencesMarkedNumber(question, ruleNum) {
  const escaped = String(ruleNum).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`\\b(?:rule|rules|section|policy)\\s+${escaped}\\b`, 'i').test(question);
}
...
export function questionReferencesRuleNumber(question, ruleNumber) {
  ...
  if (/^\d+\.\d+/.test(raw)) {
    ...
    return (
      ...
      || questionReferencesMarkedNumber(q, core)
    );
  }

  // Pure numeric rules (e.g. 10, 430) require an explicit rule/section/policy marker.
  if (/^\d+$/.test(raw)) {
    return questionReferencesMarkedNumber(q, rule);
  }
...
export function shouldUseFallbackRulebook(
  ...
  question = null, // retained for signature stability; intentionally unused
) {
  // Fall back to the governing rulebook ONLY when the primary rulebook has no
  // usable rule for this question:
  //   1. nothing was retrieved at all, or
  //   2. the best evidence score is below the confidence threshold.
  //
  // We deliberately do NOT fall back based on question length or keyword-phrase
  // overlap. A short but perfectly answerable question ("what is the slide
  // rule") must be answered from the user's OWN rulebook — never bumped to a
  // different league's book just because it produced few keywords. Falling back
  // to the wrong rulebook to avoid a possible miss is worse than the miss.
...
/**
 * Batched version of the single-node helpers above: fetch node rows, ancestor
 * chains, and child annotations for MANY rule_node_ids in one query each,
 * instead of three queries per node. Used by the retrieval hot path.
 *
 * @param {import('pg').PoolClient} dbClient
 * @param {string[]} nodeIds
 * @param {string} rulebookVersionId
 * @returns {Promise<Map<string, Object>>} nodeId → node row
 */
export async function fetchNodesByIds(dbClient, nodeIds, rulebookVersionId) {
 ...
 * @param {string[]} nodeIds
 * @param {string} rulebookVersionId
 * @returns {Promise<Map<string, Array>>} rootNodeId → ancestor rows (oldest-first order preserved: depth DESC)
 */
export async function fetchAncestorChainsBatch(dbClient, nodeIds, rulebookVersionId) {
  const map = new Map();
  if (!nodeIds.length) return map;
  const { rows } = await dbClient.query(`
    WITH RECURSIVE chain AS (
      SELECT id AS root_id, id, parent_id, node_type, rule_number, title, 0 AS depth
      FROM   rule_nodes
      WHERE  id = ANY($1::uuid[])
        AND  rulebook_version_id = $2::uuid
      UNION ALL
      SELECT c.root_id, rn.id, rn.parent_id, rn.node_type, rn.rule_number, rn.title, c.depth + 1
      FROM   rule_nodes rn
      JOIN   chain c ON rn.id = c.parent_id
      WHERE  rn.rulebook_version_id = $2::uuid
    )
    SELECT root_id, id, node_type, rule_number, title, depth
    FROM   chain
    ORDER  BY root_id, depth DESC
  `, [nodeIds, rulebookVersionId]);
  for (const row of rows) {
    if (!map.has(row.root_id)) map.set(row.root_id, []);
    map.get(row.root_id).push(row);
  }
  return map;
}
 ...
 * @param {string[]} nodeIds
 * @param {string} rulebookVersionId
 * @returns {Promise<Map<string, Array>>} parentNodeId → child annotation rows
 */
export async function fetchChildAnnotationsBatch(dbClient, nodeIds, rulebookVersionId) {
  const map = new Map();
  if (!nodeIds.length) return map;
  const { rows } = await dbClient.query(`
    SELECT id, parent_id, node_type, title, body_text, sort_order
    FROM   rule_nodes
    WHERE  parent_id = ANY($1::uuid[])
      AND  node_type IN ('comment', 'exception', 'penalty')
      AND  rulebook_version_id = $2::uuid
    ORDER  BY parent_id, sort_order ASC, created_at ASC
  `, [nodeIds, rulebookVersionId]);
  for (const row of rows) {
...
export async function fetchEvidenceBundles(dbClient, activeVersionId, question, opts = {}) {
  ...
  try {
    ...
    // Batched: one query each for node rows, ancestor chains, and child
    // annotations across all top nodes — replacing the previous 3-queries-per-node
    // (N+1) pattern. Output order still follows topNodeHits.
    const nodeIds = topNodeHits.map(h => h.rule_node_id);
    const [nodeMap, ancestorMap, childMap] = await Promise.all([
      fetchNodesByIds(dbClient, nodeIds, activeVersionId),
      fetchAncestorChainsBatch(dbClient, nodeIds, activeVersionId),
      fetchChildAnnotationsBatch(dbClient, nodeIds, activeVersionId),
    ]);

    const bundles = [];
    for (const hit of topNodeHits) {
      const node = nodeMap.get(hit.rule_node_id);
      if (!node) continue;

      const ancestors = ancestorMap.get(hit.rule_node_id) ?? [];
      const ancestorPath = formatAncestorPath(ancestors.slice(0, -1));
      const children = childMap.get(hit.rule_node_id) ?? [];
...
export async function fetchEvidenceBundlesWithFallback(dbClient, primaryVersionId, question, opts = {}) {
  ...
  // CRITICAL: only actually switch to the fallback rulebook when it is a
  // STRICTLY BETTER match than the primary. Falling back to a book that scored
  // *worse* than the user's own rulebook guarantees a wrong-league answer — the
  // exact "guarantee I'll be wrong" failure mode. A modest primary match (e.g.
  // 0.27) still beats a weaker fallback (0.22), so we keep the primary. The
  // fallback only wins when it genuinely covers the question better, or when the
  // primary retrieved nothing at all (primaryBestScore === 0).
  if (fallbackBestScore <= primaryBestScore) {
    return {
      ...primary,
      usedFallback:       false,
...
export default {
  ...
  questionReferencesMarkedNumber,
