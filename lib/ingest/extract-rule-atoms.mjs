 ...
 *
 * The UNIQUE constraint is now scoped to (rulebook_version_id, rule_number, sport)
 * so that different rulebook versions for the same league never squash each other,
 * and unnumbered rules within a version get deterministic placeholder numbers.
 *
 * Columns match RULEBOOK_DB_MIGRATION_V3.sql + migrate-v3-rules-schema.mjs.
 * `sport` defaults to 'baseball'; a future opts.sport will allow softball.
 */
const UPSERT_RULE_SQL = `
  INSERT INTO rules (league_id, rulebook_version_id, rule_number, title, body, sport)
  VALUES ($1, $2, $3, $4, $5, $6)
  ON CONFLICT (rulebook_version_id, rule_number, sport)
 ...
 *
 * For each atom:
 *   1. Assign a deterministic placeholder rule_number for unnumbered atoms so
 *      that the (rulebook_version_id, rule_number, sport) constraint never
 *      collapses distinct rules into a single row.
 *   2. UPSERT into `rules` → get the rule UUID.
 *   3. INSERT each source_id into `rule_source_links` (link_type = 'supports').
 *
 * Sequential awaiting is used deliberately:
 *   - rule_source_links inserts depend on the rule UUID from step 2.
 ...
 * @param {string}     versionId  - UUID of the rulebook_versions row for this ingest run.
...
async function insertAtoms(dbClient, leagueId, versionId, atoms, sport = 'baseball') {
  ...
  for (let i = 0; i < atoms.length; i++) {
    ...
    // ── Step 1: Assign placeholder for unnumbered atoms ───────────────────
    // Empty rule_number causes all unnumbered rules to conflict on the same
    // (rulebook_version_id, '', sport) key — every UPSERT after the first
    // would silently overwrite the previous atom.  A 1-based index placeholder
    // is stable within a single ingest run and produces a unique row per atom.
    const effectiveRuleNumber = atom.rule_number.trim() !== ''
      ? atom.rule_number
      : `[Unnumbered-${i + 1}]`;

    // ── Step 2: UPSERT the rule row ───────────────────────────────────────
    const ruleRes = await dbClient.query(UPSERT_RULE_SQL, [
      leagueId,            // $1  league_id
      versionId,           // $2  rulebook_version_id
      effectiveRuleNumber, // $3  rule_number
      atom.title,          // $4  title
      atom.body,           // $5  body
      sport,               // $6  sport
    ]);
