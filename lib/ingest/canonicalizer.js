 ...
 * @returns {{ nodes: CanonicalNode[], chunks: CanonicalChunk[], warnings: string[] }}
 */
export function canonicalizeBlocks(blocks) {
  ...
  for (const block of blocks) {
    ...
    if (heading) {
      ...
      if (heading.nodeType === 'rule') {
        ...
        if (existing && existingBodyLen > 100) {
          warnings.push(`Duplicate rule heading ${heading.ruleNumber} on page ${block.page_number}; keeping first body.`);
    ...
    if (!target || (target.node_type !== 'rule' && target.node_type !== 'subrule')) {
      warnings.push(`Orphan body block on page ${block.page_number} idx ${block.block_index}; no active rule.`);
      continue;
    }
...
export class Canonicalizer {
  ...
  /**
   * @returns {Promise<{ nodes: CanonicalNode[], chunks: CanonicalChunk[], warnings: string[] }>}
   */
  async run() {
