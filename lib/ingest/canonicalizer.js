...
/**
 * TOC lines on early pages are compact rule references with a trailing page number
 * (dot leaders or wide spacing), not substantive rule headings.
 *
 * @param {string} text
...
export function isTocHeading(text, pageNumber) {
  ...
  // Real TOC entries end with dot leaders or spaced page refs, e.g. "305. Uniforms .... 4"
  if (/\.{2,}\s*\d{1,3}\s*$/.test(line)) return true;
  if (/\s{3,}\d{1,3}\s*$/.test(line)) return true;
  return false;
}
...
export function classifyHeading(block) {
  ...
  // Lettered subsections in paragraph blocks (e.g. BAMSBL "A. Teams with nine…")
  // are rule body text, not structural subrule nodes — require heading metadata.
  if (letterMatch && (isHeadingType || role === 'subrule_heading')) {
    return {
      nodeType: 'subrule',
      ruleNumber: null,
