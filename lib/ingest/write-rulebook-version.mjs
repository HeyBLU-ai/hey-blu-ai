...
export async function writeRulebookVersion(opts = {}) {
  ...
  // Full orchestration is deferred to Step 25 (unify admin ingest on shared lib).
  // Until then, callers must use createDraftVersion() + createSourceSpans() +
  // extractRuleAtoms() individually via the CLI orchestrator (scripts/ingest-pdf.mjs).
  throw new Error(
    'writeRulebookVersion: not implemented. ' +
    'Use createDraftVersion() + createSourceSpans() + extractRuleAtoms() individually.',
  );
}
