 ...
 * @property {string[]} [parse_warnings] - Populated when dual-pass comparison detects discrepancies.
 */
...
async function parsePdf(buffer) {
  ...
  // ── Primary pass: one span per non-empty page ────────────────────────────
  const spans = [];
  let seq        = 0;
  let charOffset = 0;

  for (const page of result.pages) {
    // Normalise whitespace before building the span.
    // pdf-parse emits raw \t for column separators and preserves \n line-breaks
    // within a page.  LLMs normalize all of these to single spaces when quoting
    // text back, so we must produce the same canonical form:
    //   1. Replace tabs and newlines with a space.
    //   2. Collapse any resulting run of 2+ spaces into one space.
    // The source span text therefore contains only plain spaces — the verbatim
    // guard can then match LLM-quoted substrings reliably.
  ...
  // ── Alternate pass: split the full concatenated document text by paragraphs.
  //    Uses result.text (all pages joined by pdf-parse) so whitespace handling
  //    is different from the per-page approach.  Significant character-count
  //    divergence indicates layout complexity (columns, footnotes, etc.).
  try {
    const altParas = splitParagraphs(result.text ?? '');
    if (altParas.length > 0) {
      let altOffset = 0;
      const altSpans = altParas.map((t, i) => {
        const s = { seq: i, text: t, charStart: altOffset, charEnd: altOffset + t.length };
        altOffset += t.length + 1;
        return s;
      });
      compareExtractions(spans, altSpans);
    }
  } catch {
    // Alternate pass failure is non-fatal; primary result still returned.
  }
...
/**
 * Compare two extraction passes and annotate primary spans with parse_warnings
 * when the total character counts diverge beyond the threshold.
 *
 * Design contract:
 *   - Mutates primary spans IN PLACE (adds parse_warnings array).
 *   - Returns the primary spans so callers can chain.
 *   - A warning is added to ALL primary spans when there is a document-level
 *     discrepancy — the problem is likely systemic (e.g., a whole page was
 *     dropped), so every span gets the signal.
 *   - Exported so scripts/test-parse-warnings.mjs can call it directly
 *     without going through parsePdf / parseDocx.
 *
 * @param {SourceSpan[]} primary    - Primary extraction spans (mutated in place).
 * @param {SourceSpan[]} alternate  - Alternate extraction spans used for comparison.
 * @param {number}       [threshold=0.25] - Fractional discrepancy that triggers a warning.
 *                                          0.25 = warn when the smaller pass has <75% of
 *                                          the larger pass's character count.
 * @returns {SourceSpan[]} The primary spans, potentially with parse_warnings added.
 */
export function compareExtractions(primary, alternate, threshold = 0.25) {
  if (!primary.length || !alternate.length) return primary;

  const primaryChars   = primary.reduce((s, sp) => s + sp.text.length, 0);
  const alternateChars = alternate.reduce((s, sp) => s + sp.text.length, 0);

  if (primaryChars === 0 || alternateChars === 0) return primary;

  const larger  = Math.max(primaryChars, alternateChars);
  const smaller = Math.min(primaryChars, alternateChars);
  const ratio   = smaller / larger; // 1.0 = identical, 0.0 = completely different

  if (ratio < 1 - threshold) {
    const pct     = Math.round((1 - ratio) * 100);
    const delta   = larger - smaller;
    const warning = `dual-pass discrepancy: primary extracted ${primaryChars} chars, ` +
                    `alternate extracted ${alternateChars} chars ` +
                    `(${pct}% difference, ${delta} chars delta) — ` +
                    `review extraction; document may have complex layout or encoding issues`;
    for (const span of primary) {
      (span.parse_warnings ??= []).push(warning);
    }
  }
...
async function parseDocx(buffer) {
  // ── Primary pass: mammoth.extractRawText ─────────────────────────────────
  ...
  let charOffset = 0;
  const spans = paragraphs.map((text, i) => {
    const span = {
      seq:       i,
      text,
      heading:   null,
  ...
  // ── Alternate pass: mammoth.convertToMarkdown → strip markup ─────────────
  //    extractRawText and convertToMarkdown handle tables, footnotes, and
  //    text boxes differently.  A large character-count divergence indicates
  //    the document has structured content that extractRawText may be
  //    flattening or dropping.
  try {
    const mdResult = await mammoth.convertToMarkdown({ buffer });
    const altText  = (mdResult.value ?? '')
      .replace(/^#{1,6}\s+/gm, '')          // strip heading markers
      .replace(/\*{1,2}([^*\n]+)\*{1,2}/g, '$1')  // strip bold/italic
      .replace(/`([^`\n]+)`/g, '$1')        // strip inline code
      .replace(/!\[[^\]]*\]\([^)]*\)/g, '') // strip images
      .replace(/\[([^\]]+)\]\([^)]*\)/g, '$1'); // strip links → keep text
    const altParas = splitParagraphs(altText);
    if (altParas.length > 0) {
      let altOffset = 0;
      const altSpans = altParas.map((t, i) => {
        const s = { seq: i, text: t, charStart: altOffset, charEnd: altOffset + t.length };
    ...
    // Alternate pass failure is non-fatal.
  }
